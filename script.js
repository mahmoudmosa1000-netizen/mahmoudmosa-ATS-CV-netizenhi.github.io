...
function init() {
  ...
  startAutoSave(); updateProgress(); render(); if(typeof updateATSScore==="function") updateATSScore();
...
// ── DATUMSPICKER HELPER ──────────────────────────────────────
function makeDatePicker(prefix, existingVal, opts) {
  opts = opts || {};
  var isTo = opts.isTo || false;
  ...
  var HEUTE_MAP = { de: 'Heute', en: 'Present', ar: '\u0627\u0644\u0622\u0646' };
  var months = MONTHS_MAP[lang] || MONTHS_MAP.de;
  var heute  = HEUTE_MAP[lang]  || 'Heute';
  var initMonth = '', initYear = '', initToday = false;
  if (existingVal) {
    if (/heute|present|\u0627\u0644\u0622\u0646|current/i.test(existingVal)) {
      initToday = true;
    } else {
      var mMatch = existingVal.match(/([A-Za-z\u00e4\u00f6\u00fc\u00c4\u00d6\u00dc\u00df\u00e9\u00e0]+)/);
      var yMatch = existingVal.match(/(\d{4})/);
      if (mMatch) {
        var mStr = mMatch[1].toLowerCase().slice(0,3);
        var idx  = months.findIndex(function(m){ return m.toLowerCase().slice(0,3) === mStr; });
        if (idx >= 0) initMonth = String(idx+1).padStart(2,'0');
      }
      if (!initMonth) {
        var mNum = existingVal.match(/^(\d{1,2})[\/.\-]/);
        if (mNum) initMonth = mNum[1].padStart(2,'0');
      }
      if (yMatch) initYear = yMatch[1];
    }
  }
  var monthOpts = ['<option value="">--</option>'];
  months.forEach(function(m, i){
    var v = String(i+1).padStart(2,'0');
    monthOpts.push('<option value="'+v+'"'+(initMonth===v?' selected':'')+'>'+m+'</option>');
  });
  var todayAttr = initToday ? ' checked' : '';
  var disAttr   = initToday ? ' disabled' : '';
  var todayHtml = isTo
    ? '<label class="dp-today-label"><input type="checkbox" class="dp-today-cb" id="'+prefix+'-today"'+todayAttr+' onchange="dpToggleToday(\''+prefix+'\')">'
      + '<span class="dp-today-text">'+heute+'</span></label>'
    : '';
  return '<div class="dp-wrap" data-prefix="'+prefix+'">'
    + '<select class="dp-month" id="'+prefix+'-month" onchange="dpUpdate(\''+prefix+'\')"'+disAttr+'>'
    + monthOpts.join('')+'</select>'
    + '<input class="dp-year" id="'+prefix+'-year" type="number" min="1950" max="2040"'
    + ' value="'+initYear+'" placeholder="2024" oninput="dpUpdate(\''+prefix+'\')"'+disAttr+'>'
    + todayHtml
    + '<input type="hidden" id="'+prefix+'" value="'+(existingVal||'')+'"></div>';
}

function dpUpdate(prefix) {
  ...
  if (!monthEl || !yearEl || !hidden) return;
  var lang   = (typeof currentLang !== 'undefined') ? currentLang : 'de';
  var MONTHS_MAP = {
    de: ['Jan','Feb','Mrz','Apr','Mai','Jun','Jul','Aug','Sep','Okt','Nov','Dez'],
    en: ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'],
  ...
  if (m && y)  hidden.value = months[parseInt(m)-1] + '. ' + y;
  else if (y)  hidden.value = y;
  else if (m)  hidden.value = months[parseInt(m)-1];
  else         hidden.value = '';
  render();
}

function dpToggleToday(prefix) {
  var cb      = document.getElementById(prefix+'-today');
  var monthEl = document.getElementById(prefix+'-month');
  var yearEl  = document.getElementById(prefix+'-year');
  var hidden  = document.getElementById(prefix);
  if (!cb || !hidden) return;
  var lang  = (typeof currentLang !== 'undefined') ? currentLang : 'de';
  var heute = {de:'Heute',en:'Present',ar:'\u0627\u0644\u0622\u0646'}[lang] || 'Heute';
  if (cb.checked) {
    hidden.value = heute;
    if (monthEl) { monthEl.value=''; monthEl.disabled=true; }
    if (yearEl)  { yearEl.value='';  yearEl.disabled=true;  }
  } else {
    hidden.value = '';
...
function addEntry(type,data={}){
  ...
  if(type==='exp'){
    ...
    card.innerHTML=`
      ...
      <div class="form-row">
        <div class="form-group dp-group"><label>${t('labelFrom')}</label>${makeDatePicker('edu-from-'+id, data.from||''  , {isTo:false})}</div>
        <div class="form-group dp-group"><label>${t('labelTo')}</label>${makeDatePicker('edu-to-'+id,   data.to||''    , {isTo:true})}</div>
      </div>`;
    state.edu.push(id);
  }
...
function removeEntry(type,id){
  ...
  state[type]=state[type].filter(x=>x!==id); render(); updateProgress(); if(typeof updateATSScore==="function") updateATSScore();
}
...
function addSkill(data={}){
  ...
  card.innerHTML=`<div class="entry-card-header"><span class="drag-handle">⠿</span><span class="entry-card-label">${t('labelSkill')}</span><button class="btn-del" aria-label="Eintrag löschen" onclick="removeSkill('${id}')">×</button></div>
    ...
    <div class="form-group sk-level-group"><label>${t('labelSkillLevel')||'Level'}</label><select id="sk-pct-${id}" onchange="render()" class="sk-level-select"><option value="90" ${parseInt(data.pct||70)>=85?'selected':''}>⭐⭐⭐⭐ Experte</option><option value="70" ${parseInt(data.pct||70)>=60&&parseInt(data.pct||70)<85?'selected':''}>⭐⭐⭐ Fortgeschritten</option><option value="50" ${parseInt(data.pct||70)>=35&&parseInt(data.pct||70)<60?'selected':''}>⭐⭐ Mittelstufe</option><option value="25" ${parseInt(data.pct||70)<35?'selected':''}>⭐ Einsteiger</option></select><span class="sk-ats-badge" title="ATS-freundliches Text-Level">✓ ATS</span></div></div>`;
...
function removeSkill(id){pushUndo('skill',{name:val('sk-name-'+id),pct:val('sk-pct-'+id)});const el=document.getElementById('skill-'+id);if(el)el.remove();state.skills=state.skills.filter(x=>x!==id);render();updateProgress();if(typeof updateATSScore==="function")updateATSScore();}
...
function addProject(data={}){
  ...
  card.innerHTML=`<div class="entry-card-header"><span class="drag-handle">⠿</span><span class="entry-card-label">${t('cardProject')} #${num}</span><button class="btn-del" aria-label="Eintrag löschen" onclick="removeProject('${id}')">×</button></div>
    ...
    <div class="form-row"><div class="form-group dp-group"><label>${t('labelFrom')}</label>${makeDatePicker('proj-from-'+id, data.from||''  , {isTo:false})}</div>
    <div class="form-group dp-group"><label>${t('labelTo')}</label>${makeDatePicker('proj-to-'+id,   data.to||''    , {isTo:true})}</div></div>
...
function render(){
  ...
  const renderers={
    ...
    extraquals:()=>{
      ...
      if(licensePos==='left'){
        ...
        if(activeLic.length){leftHTML+=`<div style="margin-bottom:7px;"><div style="font-size:9px;font-weight:700;opacity:0.55;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:4px;">${t('cvLicense')||'Führerschein'}</div><div style="display:flex;flex-wrap:wrap;gap:4px;">${activeLic.map(c=>`<span style="${isPureAtsTpl?'font-size:11px;font-weight:700;':'background:rgba(255,255,255,0.18);border:1px solid rgba(255,255,255,0.3);border-radius:4px;padding:2px 8px;font-size:10px;font-weight:700;color:#fff;'}">${c}</span>`).join(isPureAtsTpl?', ':'')}</div>${licNote?`<div style="font-size:10px;opacity:0.6;margin-top:4px;font-style:italic;">${h(licNote)}</div>`:''}</div>`;}
      ...
      if(activeLic.length){s+=`<div style="display:flex;align-items:center;gap:${isPureAtsTpl?8:10}px;flex-wrap:wrap;margin-bottom:6px;"><span style="font-size:${Math.round(11*fScale)}px;font-weight:600;color:#444;">${t('cvLicense')||'Führerschein'}:</span>${isPureAtsTpl?`<span style="font-size:${Math.round(11*fScale)}px;font-weight:700;color:#222;">${activeLic.join(', ')}</span>`:`<div style="display:flex;flex-wrap:wrap;gap:5px;">${activeLic.map(c=>`<span style="background:${col};color:#fff;border-radius:5px;padding:2px 9px;font-size:${Math.round(10.5*fScale)}px;font-weight:700;">${c}</span>`).join('')}</div>`}</div>`;if(licNote)s+=`<div style="font-size:${Math.round(11*fScale)}px;color:#666;margin-bottom:6px;font-style:italic;">${h(licNote)}</div>`;}
  ...
  if(tpl==='modern'){
    ...
    const oldAts3HeaderA=paper.querySelector('.tpl-ats3-header');
    if(oldAts3HeaderA) oldAts3HeaderA.remove();
    ...
    const oldAts3HeaderB=paper.querySelector('.tpl-ats3-header');
    if(oldAts3HeaderB) oldAts3HeaderB.remove();

  }else if(tpl==='ats3'){
    // ATS KOMPAKT: die kompromisslos schlichteste Variante — linksbündig,
    // keine zentrierten Blöcke, keine Balken/Farbflächen irgendeiner Art,
    // nur dünne graue Trennlinien zwischen den Abschnitten. Entspricht am
    // ehesten dem, was Recruiting-Experten als "garantiert lesbar für
    // jedes ATS-System" empfehlen.
    paper.style.display='block';
    const contactBits3=[phone,email,address,linkedin?(linkedin.startsWith('http')?linkedin:'https://'+linkedin):'']
      .filter(Boolean).map((v,i)=>i===3?`<a href="${v}" target="_blank" style="color:#222;">${h(v)}</a>`:h(v)).join('  |  ');
    const ats3Header=`
      <div style="padding:26px 32px 12px;">
        <div style="font-family:${font};font-size:${Math.round(24*fScale)}px;font-weight:700;
          color:#161616;">${h(name)}</div>
        <div style="font-size:${Math.round(11.5*fScale)}px;color:#555;margin-top:2px;">${h(role)}</div>
        <div style="font-size:${Math.round(9.5*fScale)}px;color:#444;margin-top:8px;">${contactBits3}</div>
      </div>
    ...
    if(activeSkills.length){
      ats3Side+=atsHead3(t('cvSkills'));
      ats3Side+=`<div style="font-size:${Math.round(10.5*fScale)}px;color:#333;line-height:1.7;">${activeSkills.map(id=>h(val('sk-name-'+id))).filter(Boolean).join(', ')}</div>`;
    }
    if(activeLangs.length){
      ats3Side+=atsHead3(t('cvLanguages'));
      ats3Side+=`<div style="font-size:${Math.round(10.5*fScale)}px;color:#333;line-height:1.7;">${activeLangs.map(id=>{const n=val('ln-name-'+id),lv=val('ln-lvl-'+id);const lbl={native:t('optNative'),advanced:t('optAdvanced'),intermediate:t('optIntermediate'),basic:t('optBasic')}[lv]||lv;return n?`${h(n)}${lbl?` (${h(lbl)})`:''}`:'';}).filter(Boolean).join(', ')}</div>`;
    }
    if(komps){
      ats3Side+=atsHead3(t('cvKomps'));
      ats3Side+=`<div style="font-size:${Math.round(10.5*fScale)}px;color:#333;line-height:1.7;">${komps.split('\n').map(k=>h(k.trim())).filter(Boolean).join(', ')}</div>`;
    }
    if(hobbies){
      ats3Side+=atsHead3(t('cvInterests'));
      ats3Side+=`<div style="font-size:${Math.round(10.5*fScale)}px;color:#333;line-height:1.7;">${h(hobbies)}</div>`;
    }
    ...
    cvRight.style.display='block'; cvRight.style.width='100%';
    cvRight.style.padding='0 32px 24px';
    cvRight.innerHTML=rightHTML+ats3Side;

    const oldAts3Header=paper.querySelector('.tpl-ats3-header');
    const ats3HeaderEl=oldAts3Header||document.createElement('div');
    ats3HeaderEl.className='tpl-ats3-header';
    ats3HeaderEl.innerHTML=ats3Header;
    if(!oldAts3Header) paper.insertBefore(ats3HeaderEl, paper.firstChild);
    const oldAtsHeader3=paper.querySelector('.tpl-ats-header');
    if(oldAtsHeader3) oldAtsHeader3.remove();
    const oldAts2Header3=paper.querySelector('.tpl-ats2-header');
...
function readPhoto(file){
  ...
  reader.onload=e=>{
    ...
    img.onload=()=>{
      ...
      render();updateProgress();if(typeof updateATSScore==="function")updateATSScore();updatePhotoDesignPreview();showToast(t('toastPhotoUploaded'));
    };
    img.src=e.target.result;
  };
  reader.readAsDataURL(file);
}
function removePhoto(){state.photoData='';document.getElementById('photo-input').value='';document.getElementById('photo-preview-img').src='';document.getElementById('photo-drop-zone').style.display='block';document.getElementById('photo-preview-section').style.display='none';render();updateProgress();if(typeof updateATSScore==="function")updateATSScore();updatePhotoDesignPreview();}
...
function aiApplyToField(targetId) {
  if (!_aiLastResult) return;
  const id = targetId || 'f-summary';
  const el = document.getElementById(id);
  if (el) {
    el.value = _aiLastResult;
    el.dispatchEvent(new Event('input'));
    renderDebounced();
    updateProgress();
    if(typeof updateATSScore==="function") updateATSScore();
    if(id==='f-summary') updateCharCount();
    showToast('✓ Text übernommen!');
  }
}


// ─── INLINE KI-VERBESSERUNG ──────────────────────────────────
// Verbessert ein einzelnes Feld direkt mit CV-Kontext

async function aiImproveField(fieldId, entryType, entryId, mode) {
  const fieldEl   = document.getElementById(fieldId);
  const statusEl  = document.getElementById('ai-status-' + fieldId);
  if (!fieldEl) return;

  // Kontext sammeln — je nach Feldtyp
  const d = collectData();
  let context = {
    name:    d.name  || '',
    role:    d.role  || '',
    lang:    (typeof currentLang !== 'undefined') ? currentLang : 'de',
    existing: fieldEl.value.trim(),
  };

  if (entryType === 'exp' && entryId) {
    context.jobTitle  = val('exp-title-'   + entryId) || '';
    context.company   = val('exp-company-' + entryId) || '';
    context.from      = val('exp-from-'    + entryId) || '';
    context.to        = val('exp-to-'      + entryId) || '';
  }

  // Loading-State
  const btn = document.querySelector(`button[onclick*="aiImproveField('${fieldId}'"]`);
  const allBtns = document.querySelectorAll(`button[onclick*="'${fieldId}'"]`);
  allBtns.forEach(b => { b.disabled = true; b.style.opacity = '0.5'; });
  if (statusEl) {
    statusEl.innerHTML = '<span class="ai-inline-loading"><span class="ai-dot"></span><span class="ai-dot"></span><span class="ai-dot"></span> KI schreibt…</span>';
    statusEl.style.display = 'block';
  }

  // Prompt je nach Modus
  const langLabel = { de: 'Deutsch', en: 'Englisch', ar: 'Arabisch' }[context.lang] || 'Deutsch';
  const prompts = {
    improve: `Du bist ATS-Spezialist und Senior CV-Coach.
Verbessere diese Berufsbeschreibung für den Lebenslauf von "${context.name}" (${context.role}).
Position: ${context.jobTitle} bei ${context.company} (${context.from} – ${context.to}).

Aktueller Text:
${context.existing || '(leer — erstelle eine professionelle Beschreibung)'}

Regeln:
- Verwende starke Aktionsverben (Entwickelte, Optimierte, Leitete, Steigerte…)
- Füge messbare Ergebnisse hinzu wo möglich (z.B. "um 30%", "für 50+ Kunden")
- Nutze Bullet Points mit • 
- Max. 4 Punkte, je 1-2 Zeilen
- Sprache: ${langLabel}
- NUR der verbesserte Text, keine Erklärungen`,

    bullets: `Du bist ATS-Spezialist und Senior CV-Coach.
Erstelle 3-4 starke Bullet Points für diese Berufserfahrung.
Person: ${context.name} (${context.role})
Position: ${context.jobTitle} bei ${context.company}

Vorhandener Text (als Basis):
${context.existing || '(keine Infos — erstelle typische Aufgaben für diese Rolle)'}

Regeln:
- Jeder Bullet beginnt mit einem starken Aktionsverb
- Messbare Ergebnisse hinzufügen (Zahlen, Prozente, Umfang)
- Format: • Verb + Aufgabe + Ergebnis/Kontext
- ATS-optimiert: klare Keywords für ${context.jobTitle}
- Sprache: ${langLabel}
- NUR die Bullet Points, kein anderer Text`,
  };
  ...
  try {
    ...
    // Ergebnis in Feld einfügen
    fieldEl.value = text;
    fieldEl.dispatchEvent(new Event('input'));
    render();
    if (typeof updateATSScore === 'function') updateATSScore();

    // Status: Erfolg mit Undo-Option
    if (statusEl) {
      const prevVal = context.existing;
      statusEl.innerHTML = `<span class="ai-inline-ok">✓ Verbessert</span>
        <button class="ai-inline-undo" onclick="
          document.getElementById('${fieldId}').value=\`${prevVal.replace(/`/g,"'")}\`;
          document.getElementById('${fieldId}').dispatchEvent(new Event('input'));
          render();
          this.parentElement.style.display='none';
        ">↩ Rückgängig</button>`;
    }
    showToast('✓ KI hat den Text verbessert!');

  } catch (err) {
    console.error('[aiImproveField]', err);
    if (statusEl) {
      statusEl.innerHTML = `<span class="ai-inline-err">❌ ${err.message || 'Fehler'}</span>`;
    }
    showToast('❌ KI-Fehler: ' + (err.message || 'Unbekannter Fehler'));
  } finally {
...
// KI-Button auch für Kurzprofil (f-summary)
async function aiImproveSummary() {
  const fieldEl  = document.getElementById('f-summary');
  const statusEl = document.getElementById('ai-status-summary');
  if (!fieldEl) return;
  const d = collectData();
  const lang = (typeof currentLang !== 'undefined') ? currentLang : 'de';
  const langLabel = { de: 'Deutsch', en: 'Englisch', ar: 'Arabisch' }[lang] || 'Deutsch';
  const allBtns = document.querySelectorAll('[onclick*="aiImproveSummary"]');
  allBtns.forEach(b => { b.disabled = true; b.style.opacity='0.5'; });
  if (statusEl) {
    statusEl.innerHTML = '<span class="ai-inline-loading"><span class="ai-dot"></span><span class="ai-dot"></span><span class="ai-dot"></span> KI schreibt…</span>';
    statusEl.style.display = 'block';
  }
  const prompt = `Du bist ATS-Spezialist und CV-Coach. Schreibe ein professionelles Kurzprofil (3-4 Sätze) für:
Name: ${d.name||'–'}, Rolle: ${d.role||'–'}
...
Sprache: ${langLabel}. ATS-optimiert. Nur der Text, keine Überschriften.`;
  try {
    ...
    fieldEl.value = text;
    fieldEl.dispatchEvent(new Event('input'));
    renderDebounced(); updateProgress();
    if (typeof updateATSScore==='function') updateATSScore();
    if (statusEl) {
      statusEl.innerHTML = `<span class="ai-inline-ok">✓ Verbessert</span>
        <button class="ai-inline-undo" onclick="
          document.getElementById('f-summary').value=\`${prev.replace(/`/g,"'")}\`;
          document.getElementById('f-summary').dispatchEvent(new Event('input'));
          this.parentElement.style.display='none';
        ">↩ Rückgängig</button>`;
    }
    showToast('✓ Kurzprofil verbessert!');
  } catch(err) {
    if (statusEl) statusEl.innerHTML = `<span class="ai-inline-err">❌ ${err.message}</span>`;
  } finally {
    allBtns.forEach(b => { b.disabled=false; b.style.opacity='1'; });
  }
}
